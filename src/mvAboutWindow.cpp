...
void mvAboutWindow::drawWidgets()
{
    ...
    if (show_config_info)
    {
        ...
        ImGui::Separator();
        ImGui::Text("io.Fonts: %d fonts, Flags: 0x%08X, TexSize: %d,%d", io.Fonts->Fonts.Size, io.Fonts->Flags, io.Fonts->TexData ? io.Fonts->TexData->Width : 0, io.Fonts->TexData ? io.Fonts->TexData->Height : 0);
